...
# Verify NVIDIA GPU via system call (Windows)
gpu_info <- try(system("nvidia-smi -L", intern = TRUE), silent = TRUE)
if (inherits(gpu_info, "try-error") || length(gpu_info) == 0) {
  warning("Cannot detect NVIDIA GPU via nvidia-smi. Check CUDA/drivers.")
} else {
  cat("Detected GPUs:\n", paste(gpu_info, collapse = "\n"), "\n")
}
```
