...
glimpse(final_merged_test)
```


```{r}
#site_means = train_data_mlp %>%
  #semi_join(final_merged_test, by = "site") %>%
  #group_by(site) %>%
  #summarise()
```
...
fitted_mlp_test <- fit(final_wf_nnet, data = final_merged_test)

test_yield_prediction = predict(fitted_mlp_test, new_data = final_merged_test)

test_submission = final_merged_test %>%
  select(site) %>%
  bind_cols(test_yield_prediction) %>%
  distinct()
test_submission
