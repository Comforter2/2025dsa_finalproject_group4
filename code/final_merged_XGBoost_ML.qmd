...
final_merged = tw_sm_meta %>%
  drop_na()%>%
  select(-year, -replicate, -block, -dayl_s)
...
merged_clean <- final_merged %>%
  mutate(
    ...
    DAP            = as.numeric(date_harvested - date_planted) #Adding Days until harvest 
  )%>% select(-date_harvested, -date_planted)

merged_clean

# Factorize remaining categorical variables
...
# Write out the fully merged and cleaned CSV
write_csv(merged_factored, "../Shiny_APP/merged_factored.csv")
...
num_cols = c(
  ...
  "pHom","pk","soilpH^2","om_pct^2","p+k","norm_pk","DAP", "yield_mg_ha"
)
...
train_data %>%
  select(-hybrid, -previous_crop, -site)
...
ml_recipe = recipe(yield_mg_ha ~ ., data = train_data) %>%
    ...
    # c) remove the *original* factor columns now that they've been encoded
...
```{r}
# 0.  Load libraries
library(tidymodels)   # recipes, workflows, tuning, etc.
library(nnet)         # neural net engine
library(doParallel)   # parallel backend control
library(foreach)      # for registerDoSEQ()
library(ggplot2)      # plotting
library(dplyr)        # data manipulation
library(beepr)        # optional beep
```
...
final_merged_mlp = merged_factored %>%
  mutate(
...
mlp_spec_nnet = mlp(
  ...
  set_engine(
    "nnet",
    trace   = FALSE,
    MaxNWts = 10000          # allow plenty of weights
  ) %>%
  set_mode("regression")
